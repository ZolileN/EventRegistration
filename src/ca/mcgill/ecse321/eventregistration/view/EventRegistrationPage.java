...
public class EventRegistrationPage extends JFrame {
	...
	private static final long serialVersionUID = -8062635784771606869L;
		...
		/** Creates new from event registration page*/
		...
		private void addEventButtonActionPerformed(java.awt.event.ActionEvent evt){
			//call the controller
			EventRegistrationController erc = new EventRegistrationController();
			// JSpinner calls time and force the same time for start and end.
		...
		private void registerButtonActionPerformed(java.awt.event.ActionEvent evt){
			...
			if(selectedParticipant < 0){
				error = error + " Participants need to be selected for registration!";
			}
			if(selectedEvent < 0){
			...
			if(error.length() == 0){
				EventRegistrationController erc = new EventRegistrationController();
				try{
					erc.createRegistration(participants.get(selectedParticipant), events.get(selectedEvent));
				}catch(InvalidInputException e){
					error = e.getMessage();
				}
			}
